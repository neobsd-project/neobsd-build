...
if [ "${build_type}" = "release" ] ; then
  ...
  PKGCONG="NeoBSD_Unstable"
else
  printf "\t-b Build t ype: unstable or release"
  exit 1
fi
...
workspace()
{
  ...
  zpool destroy neobsd >/dev/null 2>/dev/null || true
  umount neobsd >/dev/null 2>/dev/null || true
  ...
  zpool create neobsd /dev/md0
  zfs set mountpoint=${release} neobsd
  zfs set compression=zstd-9 neobsd
}
...
packages_software()
{
  if [ "${build_type}" = "unstable" ] ; then
    cp pkg/NeoBSD_Unstable.conf ${release}/etc/pkg/NeoBSD.conf
  fi
  cp /etc/resolv.conf ${release}/etc/resolv.conf
  mkdir -p ${release}/var/cache/pkg
...
user()
{
  ...
  -c "NeoBSD Live User" -d "/usr/home/${liveuser}"\
...
extra_config()
{
  ...
  mkdir -p ${release}/usr/local/share/neobsd
  echo "${desktop}" > ${release}/usr/local/share/neobsd/desktop
  echo "${liveuser}" > ${release}/usr/local/share/neobsd/liveuser
  ...
  # default NeoBSD to local time instead of UTC
...
uzip()
{
  ...
  mkdir "${cdroot}/data"
  zfs snapshot neobsd@clean
  zfs send -c -e neobsd@clean | dd of=/usr/local/neobsd-build/cdroot/data/system.img status=progress bs=1M
}
...
ramdisk()
{
  ...
  sed "s/@VOLUME@/NEOBSD/" "init.sh.in" > "${ramdisk_root}/init.sh"
...
boot()
{
  ...
  mkdir ${cdroot}/etc
  cd ${cwd} && zpool export neobsd && while zpool status neobsd >/dev/null; do :; done 2>/dev/null
}
