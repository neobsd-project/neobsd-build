...
lightdm_setup()
{
  ...
  if [ "${desktop}" == "xfce" ] ; then
    ...
    sed -i '' "s@#user-session=default@user-session=gnome@" ${release}/usr/local/etc/lightdm/lightdm.conf
  fi
  setup_xinit
...
setup_xinit()
{
  if [ "${desktop}" == "mate" ] ; then
    ...
    echo "exec ck-launch-session gnome-session" > ${release}/usr/home/${liveuser}/.xinitrc
    echo "exec ck-launch-session gnome-session" > ${release}/root/.xinitrc
  fi
}
